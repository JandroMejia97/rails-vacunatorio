class VaccinesController < ApplicationController
  include SessionsHelper, VaccinesHelper
  before_action :set_vaccine, only: %i[ show edit update destroy ]

  # GET /vaccines or /vaccines.json
  def index
    @campaigns = Campaign.where("end_date <= ?", Date.today).order(:start_date)
  end
  ...
  # GET /vaccines/new
  def new
    @vaccine = Vaccine.new(campaign_id: params[:campaign])
  end
  ...
  private
    ...
    def vaccine_params
      params.require(:vaccine).permit(:name, :campaign_id, :stock, :number_of_doses)
    end
end
